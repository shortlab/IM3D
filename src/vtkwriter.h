/******************************************************************************
  Module Name : vtkwriter.h
  Module Date : 08/20/2014
  Module Auth : Yonggang Li, ygli@theory.issp.ac.cn

  Description : VTK file format writer.
                This file contains function prototypes for writing out point
                meshes, unstructured meshes, rectilinear meshes, regular meshes,
                and structured/curvilinear meshes into files that can later be
                red by VisIt/ParaView.
                ...
                However: the VisIt/ParaView reader will determine that the data is truly
                two-dimensional and visualize it as a two-dimensional dataset.

                All writers have an ASCII vs Binary decision.  The trade-offs are the
                ...
                interpolation problems appear along grid boundaries.

  Others :
      Refers to visit_writer.
...
/*-----------------------------Functions-----------------------------*/
/* Store the results of the simulation (arrays with distribution
   of implanted ions, defects etc.) in msh format */
int store_results_vtk (char *base_name);

/* Writes the designated array of Count elements into a vtk file */
int write_int_array_to_vtk_file (char *file_name, int mat_i, int *source_array_total,
                                 int *source_array_part[], int count, int n_element);

/* Writes the designated array of Count elements into a vtk file */
int write_double_array_to_vtk_file (char *file_name, double *source_array1,
                                    double *source_array2, int count);

/* Calculate the average value of the nodes */
float average_node_value_int (int index, int *element_non_zero, int *source_array);

/* Calculate the average value of the nodes */
float average_node_value_double (int index, int *element_non_zero, double *source_array);
...
/*=============================================================================
  ...
  Example:
      ...
      because VisIt/ParaView is not able to correctly determine the connectivity
      of a dataset when points are duplicated.
...
/*=============================================================================
  ...
  Arguments:
      ...
      y          An array of size dims[1] that contains the y-coordinates.
      z          An array of size dims[2] that contains the z-coordinates.
      nvars      The number of variables.
      vardim     The dimension of each variable.  The size of vardim should
                 be nvars.  If var i is a scalar, then vardim[i] = 1.
                 If var i is a vector, then vardim[i] = 3.
