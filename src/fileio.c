...
int write_int_array_to_cfg_file (char *file_name, int mat_i, int *source_array_total,
    ...
    if (flag == NULL) return -5108;  /* cannot allocate memory */
    count_non_zero = 0;
    for (i=0; i<count; i++) {
...
int read_config_file_data (char *par_name, char *par_value) {
    ...
    if (strcmp (par_name, "OutputFileBaseName") == 0) {  /* ... */
        ...
        if (OutputFileBaseName == NULL) return -5011;
...
int read_float_block (char *file_name, int offset, int count, float *array) {
    ...
    if (fp == NULL) return -5014;  /* cannot open file */
...
int split_single_input_file (char *file_name) {
    ...
    strcpy (ConfigFileName, "temp_configfile.iran3d");
    /* filename of the file that define the structure of the target */
    strcpy (TargetStructureFileName, "temp_structfile.iran3d");
    ...
    strcpy (MaterialsFileName, "temp_matfile.iran3d");
    ...
    if (line == NULL) return -5023;  /* out of mem */
