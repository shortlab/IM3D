...
int write_int_array_to_msh_file (char *file_name, int mat_i, int *source_array_total,
    ...
    if (element_non_zero == NULL) return -5101;  /* cannot allocate memory */
    element_non_zero_index = (int*) calloc (count, sizeof (int));
    if (element_non_zero_index == NULL) return -5101;  /* cannot allocate memory */
    for (i=0; i<count; i++) {
    ...
    if (node_non_zero == NULL) return -5102;  /* cannot allocate memory */
    node_non_zero_index = (int*) calloc (node_count, sizeof (int));
    if (node_non_zero_index == NULL) return -5102;  /* cannot allocate memory */
    num_nodes = 0;
...
int write_double_array_to_msh_file (char *file_name, double *source_array1,
    ...
    /* only count and output the non-zero elements */
    element_non_zero = (int*) calloc (count, sizeof (int));
    if (element_non_zero == NULL) return -5104;  /* cannot allocate memory */
    element_non_zero_index = (int*) calloc (count, sizeof (int));
    if (element_non_zero_index == NULL) return -5104;  /* cannot allocate memory */
    for (i=0; i<count; i++) {
        element_non_zero[i] = -1;
    }
    num_elements = 0;
    ...
    /* count notes */
    node_non_zero = (int*) calloc (node_count, sizeof (int));
    if (node_non_zero == NULL) return -5105;  /* cannot allocate memory */
    node_non_zero_index = (int*) calloc (node_count, sizeof (int));
    if (node_non_zero_index == NULL) return -5105;  /* cannot allocate memory */
    num_nodes = 0;
    for (i=0; i<node_count; i++) {
        get_node_XYZ (i, &x, &y, &z);
...
/*=============================================================================
  ...
  Description   : Calcualtes index for accessing one-dimensional node arrays
