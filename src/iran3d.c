...
/*=============================================================================
  ...
  Notes :
      Function call:
        iran3d - int display_startup_message ();
    ...
    if (cmd_result == 0) {  /* normal simulation */
        ...
        if (my_node == ROOT) {
            ...
            case 0 :  /* iradina */
                ...
                break;
            case 1 :  /* cfg */
                store_results_cfg (OutputFileBaseName);  // ygli
                break;
            case 2 :  /* msh */
                store_results_msh (OutputFileBaseName);  // ygli
                break;
            default :
                printf ("ERROR: ouptput format is not existent.");
                break;
            }
        ...
        if (cmd_result < 0) {  /* an error occured */
            ...
            case 3 :  /* convert material to element file */
                printf ("\nYou are using iran3d to convert a material based target definition to an element based one.\n\n");
...
int display_startup_message () {
    if (print_level >= -1) {
        printf ("************************************************************************\n");
        printf ("IRAN3D: Ion IRadiation of Nanostructured materials\n");
        printf ("        -- a 3D parallel Monte Carlo simulation code\n\n");
        printf ("IRAN3D Version %i.%i.%i%s, ", VERSION, SUBVERSION, SUBSUBVERSION, RELEASESTRING);
